...
export async function GET(
  ...
  try {
    const [sharedFiles, totalCount] = await prisma.$transaction([
      prisma.fileEntry.findMany({
        where: {
          shares: {
            some: {
              sharedWithPK: tokenPayload.wallet,
            },
          },
          deleted: false,
        },
    ...
    const fileEntryResponses: FileEntryResponse[] = sharedFiles.map((file: PrismaFileEntry) => {
      const baseEntry: Omit<FileEntryResponse, 'type' | 'mimeType' | 'size' | 'url'> = {
        ...
        uploadedSize: file.uploadedSize || 0,
      };
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to fetch shared files' },
      { status: 500 }
    );
  }
}
